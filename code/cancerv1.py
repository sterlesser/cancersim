...
class CancerSim:
    ...
    def _set_radii(self):
        """ Adjust all of the radii to their average """

        for cell in self.cells:
            avg = 0.0
            for neigh in self.links.get_neighbors(cell):
                avg += 0.5*self.links.get_link(cell,neigh).extension
            cell.radius = avg
    ...
    def force_func(self,cell1,cell2):
        r""" the native force function between two positions
        from the potential

        .. math::

            V = \left(\frac{a}{r}\right)^4 \left( 1 - \frac{r}{\xi} \right)^2

            F = 2 \frac{a^4}{\xi^2 r^6} \left( 2 \xi^2 - 3 \xi r + r^2 \right) \vec x


        We get the above force.
        """
        x1 = cell1.pos
        x2 = cell2.pos
        disp = x1 - x2
