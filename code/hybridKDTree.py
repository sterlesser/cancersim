...
from numpy import asarray, amax, sum, inf, abs
...
def minkowski_distance_p(x,y,p=2):
    ...
    x = asarray(x)
    y = asarray(y)
    #print "Fed in arguments to p have form: {}, {}".format(x.shape, y.shape)
    if p==inf:
        return amax(abs(y-x),axis=-1)
    elif p==1:
        return sum(abs(y-x),axis=-1)
    else:
        return sum(abs(y-x)**p,axis=-1)
        
def minkowski_distance(x,y,p=2):
    """Compute the L**p distance between x and y"""
    x = asarray(x)
    y = asarray(y)
    #print "Fed in arguments have form: {}, {}".format(x.shape,y.shape)
    if p==inf or p==1:
